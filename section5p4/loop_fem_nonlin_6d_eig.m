...
% Global variables
% --->
global param num vec_a cons_c data_x lin_eq mean_Young mean_Poisson
...
fprintf(' size of elastic body = %3.2f mm x %3.2f mm \n',...
    Length, Width);
fprintf(' mesh = %g x %g \n',  NXE, NYE);
fprintf(' for model-based solver \n');
fprintf('     initial material constants: \n');
for i=1:2
    fprintf('     Young = %3.5f MPa ;  Poisson = %3.5f \n',...
        mean_Young(i) * param_scale, mean_Poisson(i));
end
fprintf(' compare displacement at right bottom node, y-direction \n');

incr_load_factor = 1.00 / num_Iter;
sol_u   = zeros(nd, 1);
